...
#' @export
count_byte <- function(x, needle = as.raw(0x41)) count_byte_extendr(check_raw(x), normalize_needle(needle))

#' Find byte occurrences
#' @inheritParams count_byte
#' @return A data frame with zero-based `position` and integer `byte` columns.
#' @export
find_byte <- function(x, needle = as.raw(0x41)) find_byte_extendr(check_raw(x), normalize_needle(needle))
